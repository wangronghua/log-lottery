...
var stages = [trailsStage, mainStage];

//随机文字烟花内容
const randomWords = ["恭喜抽中大奖"];
const wordDotsMap = {};
...
const qualitySelector = () => +3;// +store.state.config.quality;
const shellNameSelector = () => "Random";//store.state.config.shell;
// Convert shell size to number.
const shellSizeSelector = () => +6//+store.state.config.size;
const finaleSelector = () => store.state.config.finale;
const skyLightingSelector = () => +0;//+store.state.config.skyLighting;
const scaleFactorSelector = () => 1.0;//store.state.config.scaleFactor;
...
function init() {
	...
	var item = document.querySelector(".loading-init");
	if(item){
		item.remove();
	}
...
function startPlayYanhua(){
	if(!hasInited){
		hasInited=true;
		//closebtn
		init();

		const element = document.querySelector('#closebtn');
		if (element) {
